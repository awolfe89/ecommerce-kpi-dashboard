...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { type, model, data } = JSON.parse(event.body);
    ...
    let prompt = '';
    if (type === 'monthly') {
      prompt = generateMonthlyReportPrompt(data);
    } else if (type === 'comparison') {
      prompt = generateComparisonReportPrompt(data);
    } else {
      return {
        statusCode: 400,
    ...
    const openaiResponse = await callOpenAI(prompt, model || 'gpt-4-turbo');
    
    // Process and format the response
    const formattedReport = formatReport(openaiResponse, data, type);
...
/**
 * Generates a prompt for the monthly report
...
/**
 * Generates a prompt for the website comparison report
 * @param {Object} data - The data context with multiple websites
 * @returns {string} - The generated prompt
 */
...
/**
 * Call the OpenAI API to generate the report content
 ...
 * @param {Object} data - The original data context
